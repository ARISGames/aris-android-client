...
//	public Collection<Log> logs = new LinkedList<>();
	...
	public void updateLogs(List<Log> newLogs) {
		...
		for (Log newLog : newLogs) {
			...
			if (logs.get(newLogId) == null) logs.put(newLogId, newLog); // setObject:newLog forKey:newLogId);
		}
	...
	public void playerViewedContent(String content, long content_id)
	{
		if(mGame.network_level.contentEquals("LOCAL"))
		{
			if(content.contentEquals("PLAQUE"))        mGamePlayAct.mServices.logPlayerViewedPlaqueId(content_id);
			if(content.contentEquals("ITEM"))          mGamePlayAct.mServices.logPlayerViewedItemId(content_id);
			if(content.contentEquals("DIALOG"))        mGamePlayAct.mServices.logPlayerViewedDialogId(content_id);
			if(content.contentEquals("DIALOG_SCRIPT")) mGamePlayAct.mServices.logPlayerViewedDialogScriptId(content_id);
			if(content.contentEquals("WEB_PAGE"))      mGamePlayAct.mServices.logPlayerViewedWebPageId(content_id);
			if(content.contentEquals("NOTE"))          mGamePlayAct.mServices.logPlayerViewedNoteId(content_id);
			if(content.contentEquals("SCENE"))         mGamePlayAct.mServices.logPlayerViewedSceneId(content_id);
		}
		...
		if(content.contentEquals("DIALOG_SCRIPT")) this.addLogType("VIEW_DIALOG_SCRIPT", content_id, 0);
